...
class Preprocessor():
    ...
    def __init__(self, db_dir='Data/dev_database', db_content=True):
    ...
    def preprocess_with_custom_dict(self, entry: dict, custom_entities: set, verbose: bool = False):
        """Tokenize, lemmatize, and lowercase question, ensuring custom entities are preserved."""
        question = ' '.join(quote_normalization(entry['question_toks']))
        doc =self.nlp(question)

        custom_entities_tokens = {
            entity.lower(): [w.text.lower() for s in self.nlp(entity).sentences for w in s.words]
        ...
        for sentence in doc.sentences:
            ...
            while index < len(words):
                token_text = words[index].text.lower()

                matched_entity = None
                for entity, entity_tokens in custom_entities_tokens.items():
                    index_copy = index
                    match_sequence = [token_text] 
                    matched_entity = None 

                    if token_text == ''.join(entity_tokens):
                        matched_entity = entity
                        print(f"Matched entity: {matched_entity} (exact match)")
                    else:
                        for i in range(1, 6): 
                            if index_copy + i < len(words):
                                match_sequence.append(words[index_copy + i].text.lower())
                                if ''.join(match_sequence) == ''.join(entity_tokens):
                                    matched_entity = entity
                                    print(f"Matched entity: {matched_entity} (partial match with continuation)")
                                    break 
                            else:
                                break

                    if matched_entity:
                        raw_toks.append(matched_entity)
                        processed_toks.append(matched_entity)  
                        upos_tags.append("PROPN")  
                        xpos_tags.append("NNP")  
                        feats_tags.append(None)
                        index += len(match_sequence)  
                        break  

                else:
             
                    raw_toks.append(token_text)
                    processed_toks.append(words[index].lemma.lower()) 
                    upos_tags.append(words[index].upos)
                    xpos_tags.append(words[index].xpos)
                    feats_tags.append(words[index].feats)
                    index += 1

        entry['raw_question_toks'] = raw_toks
        entry['processed_question_toks'] = processed_toks
        entry['upos_tags'] = upos_tags
        entry['xpos_tags'] = xpos_tags
        entry['feats_tags'] = feats_tags

        if verbose:
            print("Raw Tokens:", raw_toks)
            print("Processed Tokens:", processed_toks)
            print("UPOS Tags:", upos_tags)
            print("XPOS Tags:", xpos_tags)
            print("Feats Tags:", feats_tags)
    ...
    def preprocess_database(self, db: dict,custom_entities: set, verbose: bool = False):
        ...
        for tab in db['table_names']:
            ...
            while index < len(words):
                ...
                for entity, entity_tokens in custom_entities_tokens.items():
                    ...
                    if matched_entity:
                        tab_toks.append(matched_entity)
                        index += len(match_sequence)
                        break

                else:
                    cleaned_token = words[index].lemma.lower().replace('-', '')
                    tab_toks.append(cleaned_token)
                    index += 1

            table_toks.append(tab_toks)
            table_names.append(" ".join(tab_toks))
        ...
        for _, c in db['column_names']:
            ...
            while index < len(words):
                ...
                for entity, entity_tokens in custom_entities_tokens.items():
                    ...
                    if matched_entity:
                        col_toks.append(matched_entity)
                        index += len(match_sequence)
                        break

                else:
                    cleaned_token = words[index].lemma.lower().replace('-', '')
                    col_toks.append(cleaned_token)
                    index += 1

            column_toks.append(col_toks)
            column_names.append(" ".join(col_toks))
        ...
        print(f"table_mat.shape:{tab_mat.shape}")
        ...
        print(f"col_mat.shape:{col_mat.shape}")
        ...
        cols, tabs = list(zip(*[(x, column2table[x]) for x in range(1, c_num)]))  # range(1, c_num) 忽略 *
        ...
        if len(db['primary_keys']) > 0:
            for pk in db['primary_keys']:
                if isinstance(pk, list):  
                    for col in pk:
                        tab = column2table[col]
                        col_tab_mat[col, tab], tab_col_mat[tab, col] = 'column-table-pk', 'table-column-pk'
                else:  
                    col = pk
                    tab = column2table[col]
                    col_tab_mat[col, tab], tab_col_mat[tab, col] = 'column-table-pk', 'table-column-pk'
    ...
    def preprocess_question(self, entry: dict, db: dict,custom_entities: set, verbose: bool = False):
        ...
        entry = self.preprocess_with_custom_dict(entry, custom_entities, verbose=verbose)
        toks = entry['processed_question_toks']
        ...
        if verbose:
            ...
            print('UPos tags:', ' '.join(entry['upos_tags']), '\n')
            print('XPos tags:', ' '.join(entry['xpos_tags']), '\n')
            # print('Feats tags:', ' '.join(entry['feats_tags']), '\n')
        return entry
    ...
    def schema_linking(self, entry: dict, db: dict,custom_entities: set, verbose: bool = False):
        ...
        column_matched_pairs = {'partial': [], 'exact': [], 'value': [], 'description': []}
