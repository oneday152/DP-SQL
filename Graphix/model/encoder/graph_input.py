...
from Graphix.model.model_utils import rnn_wrapper, lens2mask, PoolingFunction
...
class RGATInputLayer(nn.Module):
    def __init__(self, args):
        ...
        """
        super(RGATInputLayer, self).__init__()
        self.input_dim = args.input_dim
        self.hidden_size = args.gnn_hidden_size
        self.feat_drop = args.feat_drop

        self.linear1 = nn.Linear(self.input_dim, self.hidden_size // 2)
        ...
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=self.feat_drop)

    def forward(self, batch):

        x = batch.graph.node_features  
        x = self.linear1(x)
        x = self.norm1(x)
        x = self.relu(x)
        x = self.dropout(x)
