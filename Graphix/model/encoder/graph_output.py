...
from Graphix.model.encoder.functions import scaled_exp, div_by_z, src_dot_dst
...
class MeanLayer(nn.Module):
    ...
    def forward(self, inputs, mask):
        ...
        mask = mask.view(-1)
        unique_masks = mask.unique(sorted=True)  
        num_groups = unique_masks.size(0) 
        
        grouped_means = []
        
        for group_id in unique_masks:
            group_indices = (mask == group_id).nonzero(as_tuple=True)[0]
            group_vectors = inputs[group_indices]
            group_mean = group_vectors.mean(dim=0)
            grouped_means.append(group_mean)
        
        return torch.stack(grouped_means, dim=0)

    
class RGATOutputlayer(nn.Module):
    def __init__(self, args):
        super(RGATOutputlayer, self).__init__()
        self.hidden_size = args.gnn_hidden_size
        self.output_size = args.output_size
        self.affine = nn.Linear(self.hidden_size, self.hidden_size//2)
        self.mean = MeanLayer()
        self.ffn = nn.Sequential(nn.Linear(self.hidden_size//2, self.hidden_size//4), nn.ReLU(), nn.Linear(self.hidden_size//4, self.output_size))

    def forward(self, x,batch):

        x = self.affine(x)
        print(x.shape)
        x = self.mean(x, batch.graph.mask)
        print(x.shape)
        x = self.ffn(x)
        return x
